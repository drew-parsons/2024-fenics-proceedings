...
same curve, with small deviation seen only at 3V, that may be attributed simply to the
integer resolution of the simulation.  The time per iteration and the total number of
...
\caption{Best fixed rate multiplier $r$ for each voltage, giving lowest
  ...
  0.38 \ln(1 + 0.1 V)$ fitted to trivial scaling data. Grey lines
...
\cite{VugrinSwilerRobertsStuckyMackSullivan2007}. The fitted curve is
shown (
...
These final performance tests used adaptive $r$ coefficients taken
from trivial scaling ($A=3.8$, $B=0.1 \textrm{ V}^{-1}$).
