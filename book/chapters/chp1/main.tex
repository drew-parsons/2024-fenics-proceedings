...
\begin{table}[]
    ...
    \begin{tabular}{c|c|c|c|c|c}
        ...
        \hline
         ...
         & & & & & $E_\text{cross, set}= 0$
    \end{tabular}
    \caption{Values for the model variables.}
    \label{tab:varaibles}
\end{table}
...
\hline
KFR & $\displaystyle F_{g,f}^i(\beta(\sqrt{2 E_{ff} + 1} - 1 - s_{hom}) + 1)^{1/3}$ & $\displaystyle F_{g,r}^i(\beta(\sqrt{2 E_{ff} + 1} - 1 - s_{hom}) + 1)^{1/3}$ & $\displaystyle F_{g,c}^i(\beta(\sqrt{2 E_{ff} + 1} - 1 - s_{hom}) + 1)^{1/3}$ \\
\hline
GEG & $\displaystyle \frac{1}{\tau}\left(\frac{F_{g,f,max} - F_{g,f}^i}{F_{g,f,max} - 1}\right)^\gamma(F_{e, f}^i - \lambda^\text{crit}) + F^i_{g, f}$ & $\displaystyle 1$ & $\displaystyle 1$ \\
\hline
GCG & $\displaystyle 1$ & $\displaystyle  \frac{1}{\tau}\left(\frac{F_{g,c,max} - F_{g,c}^i}{F_{g,c,max} - 1}\right)^\gamma(\tr(\mathbf{M}) - p^\text{crit}) + F^i_{g, c}$ & $\displaystyle 1$ \\
\hline
KOM & $\displaystyle \begin{cases}
        F_{g,f}^{i}k_{ff}\frac{f_{ff,\text{max}}\Delta t_\text{growth}}{1 + \exp(-f_f(s_l-s_{l,50}))} + 1, \qquad s_l \geq 0\\
        F_{g,f}^{i}\frac{-f_{ff,\text{max}}\Delta t_\text{growth}}{1 + \exp(f_f(s_l+s_{l,50}))} + 1, \qquad s_l < 0
    \end{cases} $ & $\displaystyle \begin{cases}
        F_{g,c}^{i}\sqrt{k_{cc}\frac{f_{cc,\text{max}}\Delta t_\text{growth}}{1 + \exp(-c_f(s_t-s_{t,50}))} + 1}, \qquad s_t \geq 0\\
        F_{g,c}^{i}\sqrt{\frac{-f_{cc,\text{max}}\Delta t_\text{growth}}{1 + \exp(c_f(s_t+s_{t,50}))} + 1}, \qquad s_t < 0 
    \end{cases} $ & $\displaystyle \begin{cases}
        F_{g,c}^{i}\sqrt{k_{cc}\frac{f_{cc,\text{max}}\Delta t_\text{growth}}{1 + \exp(-c_f(s_t-s_{t,50}))} + 1}, \qquad s_t \geq 0\\
        F_{g,c}^{i}\sqrt{\frac{-f_{cc,\text{max}}\Delta t_\text{growth}}{1 + \exp(c_f(s_t+s_{t,50}))} + 1}, \qquad s_t < 0 
    \end{cases} $ \\
...
\end{sidewaystable}
\restoregeometry
The parameters $T$, $\beta$, $\tau$, and $\Delta t$, simply determine the rate of growth and can be tuned to \replaced{match}{math} the growth rate of data obtained from experiments. In LT2, $\sigma_{p,0}$ and $\sigma_{a,0}$ are set points for the passive and active fiber stress at equilibrium. For KFR, $s_{hom}$ is the strain set point. For GEG and GCG, $F_{g,f,max}$ and $F_{g,c,max}$ is the maximum amount of growth allowed to occur. $\mathbf{M}$ is the Mandel stress, which is defined as 
\begin{equation*}
    \mathbf{M} = \mathbf{F}^\top \mathbf{P}
\end{equation*}
and $p^\text{crit}$ is the stress set point. $\lambda^\text{crit}$ is the strain set point. For KOM, $k_{ff}$ and $k_{cc}$ are defined as
\begin{align*}
...
\begin{align*}
    ...
    s_t &= \min(E_\text{cross, max}) - E_\text{cross, set}
\end{align*}
where $E_{ij}$ is the Lagrange strain tensor, and $E_{ff}$ is the strain in the fiber direction, and $E_\text{cross, max}$ is the maximum algebraic maximum principle strain of the matrix (see \cite{Witzenburg2018})
\begin{equation*}
    ...
    \end{pmatrix}
\end{equation*}
and $E_{ff, \text{set}}$ and $E_\text{cross, set}$ are set points. \par
...
First, we note that LT2 displayed runaway growth, as was also found in \cite{Witzenburg2018}. GCG seems to be converging to $F_{g,c,max}$, and GEG seem to have converged because it reached $\lambda^\text{crit}$. The reason GCG is growing oppositely compared to GEG is probably because GEG was created to model growth triggered by volume overload while GCG was created to capture growth triggered by pressure overload. KFR grew an equal amount in each direction, and stabilized. It is not clear under what conditions KFR should be stable because $s_{hom}$ is the same in each direction. When we ran simulations with other boundary conditions, it was not stable. The KOM model was stable for many different types of boundary conditions, but is the most computationally expensive model to run.\par 
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{10p_stretch_fiber.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{10p_compression_fiber.png}
