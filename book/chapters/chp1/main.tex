...
\abstract{ The nonlinear Poisson-Boltzmann model of electrolyte
  ...
  Our implementation
  allows for other nonelectrostatic molecular interactions that are important
  for modelling real chemical and electrochemical systems, in
  particular steric forces due to finite ion sizes. The combination enables
...
\end{equation}
for all test functions $b_i$. The strong form of the classical Boltzmann equation can then be obtained, $c_i(x)=c_{i\infty}\exp(-z_i e \psi(x)/kT)$.
...
For more complex non-planar geometries, adaptive remeshing
would be suitable, controlled by the magnitude of concentration
gradients or the gradient of the electrostatic potential (i.e.\ the
electric field).
...
FEniCS-X \citep{baratta2023dolfinx}, using continuous 
...
up to 1.9V. Nevertheless \figref{fig_classical_PB}b demonstrates the
...
\begin{figure}
\centering
\includegraphics[width=0.9\linewidth]{throttling3.eps}
...
in  \figref{fig:best_throttle_rate}. The trend suggests that an
