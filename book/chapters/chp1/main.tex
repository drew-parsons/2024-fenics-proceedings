...
\end{equation}
where $\mathbf{F}_e$ is the elastic deformation and $\mathbf{F}_g$ is the plastic deformation that represents growth. The early adopters of the multiplicative split of elastic and plastic deformations were by \citep{Kondaurov1987}, \citep{Takamizawa1990}, and \citep{Rodriguez1994} to study growth and remodelling of biological tissue. \par 
...
\end{equation*}
where $F^\mathrm{inc}_{g, i}$ for  $i = \{f, c, n\}$ are functions of either stress or strain, and $\mathbf{e}_i$ for $i = {\{f, c, n\}}$ are orthonormal basis vectors in the fiber, crossfiber, and normal directions respectively. The initial growth tensor, $\mathbf{F}_g^0$, is set to the identity tensor. The equations are then furnished with boundary conditions that that describe the load and displacement of the boundary,
...
\emph{Numerical simulations:} We ran two simulations, one with a 10\% stretch and one with a 10\% compression which corresponds to $g_D = 0.1$ and $g_D = -0.1$ respectively. In the stretch simulation $F_{g,c,\mathrm{max}}$ was set to 1.2 and 0.8 in the compression simulation. We set $\mu = 15$ kPa and $\kappa = 100$ kPa.
...
\begin{table}[htbp]
    ...
    \end{tabular}
    \caption{Model parameters for the growth models. All units are SI units except for stress which is in kilo Pascals. $T$, $\beta$, and $\tau$ are dimensionless and determine the speed of growth. $\Delta t$ has units day, while $f_\mathrm{ff,max}$ and $f_\mathrm{cc,max}$ have units 1/day.}
